...
t.test('run() without threads limit', async (t) => {
  ...
  t.same(completed, wantedResult, 'all tasks completed in proper order');

  t.equal(
    performance.max,
    distinctTargetIdsCount,
    '`performance.max` should be `' +
      distinctTargetIdsCount +
      '` (equal to number of distinct `targetId`) (' +
      performance.max +
      ')'
  );
  t.ok(
    performance.avg > distinctTargetIdsCount - 3.5,
    '`performance.avg` should be greater than `' +
      (distinctTargetIdsCount - 3.5) +
      '` (~number of distinct `targetId`) (' +
      performance.avg +
      ')'
  );
});
...
t.test('run() with 2 max threads', async (t) => {
  ...
  t.equal(performance.max, 2, '`performance.max` should be `2` (' + performance.max + ')');
  t.ok(performance.avg > 1.5, '`performance.avg` should be greater than `1.5` (' + performance.avg + ')');
});
...
t.test('run() with 3 max threads', async (t) => {
  ...
  t.equal(performance.max, 3, '`performance.max` should be `3` (' + performance.max + ')');
  t.ok(performance.avg > 2, '`performance.avg` should be greater than `2` (' + performance.avg + ')');
});
...
t.test('run() with 5 max threads', async (t) => {
  ...
  t.equal(performance.max, 5, '`performance.max` should be `5` (' + performance.max + ')');
  t.ok(performance.avg > 3.8, '`performance.avg` should be greater than `3.8` (' + performance.avg + ')');
});
...
t.test('run() with 2 threads on modifying queue', async (t) => {
  ...
  t.pass('run() executed sucessfully');
  t.match(completed, wantedResult, 'all tasks completed in proper order');
...
t.test('run() with 3 threads on infinite queue', async (t) => {
  ...
  t.ok(performance.avg > 2.5, '`performance.avg` should be greater than `2.5` (' + performance.avg + ')');
