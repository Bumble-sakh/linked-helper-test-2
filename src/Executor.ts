...
export default class Executor implements IExecutor {
  ...
  public stop() {
    ...
    this.performanceReport.min = this.executeData.performanceData
    ...
    this.performanceReport.max = this.executeData.performanceData.reduce((max, record) => {
    ...
    this.performanceReport.avg = this.executeData.performanceData.reduce((avg: number, record) => {
  ...
  public async executeTask(task: ITask) {
    ...
    if (running[targetId]) {
      throw new Error(
        `cannot execute task ${targetId}:` + `${task.action}: task with the same targetId=${targetId} is running`
      );
    }
    ...
    this.recordPerformance(true);
    console.log(`task: ${task.targetId} - ${task.action}`);
  }
